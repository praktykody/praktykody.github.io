...
- funkcję ```initButtons()``` generującą przyciski 
- obiekt ```buttons:{}``` którym będziemy zmieniać przyciski (aktywne, nieaktywne)
- ```gamePlane``` - obiekt DOM, aby szybciej do niego się odnosić (zamiast ```document.querySelector(".gamePlane")```)  
```js
const game = {
